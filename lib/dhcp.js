...
Server.prototype = {
  ...
  _isMyAddress: function(clientMAC, req) {

    /*
     * Check if the ciaddr of a DHCPREQUEST is an address administered by
     * this DHCP server
     */

     // If existing lease for a mac address is present and equal to ciaddr
    if (this._state[clientMAC] && this._state[clientMAC].address === req.ciaddr) {
      return true;
    }
    ...
    if (typeof _static === "function") {
      const staticResult = _static(clientMAC, req);
       return staticResult === req.ciaddr? true : false;
    } else if (_static[clientMAC] === req.ciaddr) {
      return true;
    }

    // Is it within the configured range?
    const _tmp = this.config('range');
    const firstIP = Tools.parseIp(_tmp[0]);
    const lastIP = Tools.parseIp(_tmp[1]);

    if (req.ciaddr >= firstIP && req.ciaddr <= lastIP) {
      return true;
    }
},
  ...
  handleRequest: function(req) {
    ...
    // A renew or rebind has the ciaddr set - check if this is ours before responding
    if (!req.ciaddr || this._isMyAddress(req.chaddr, req)) {
      const lease = this._state[req.chaddr] = this._state[req.chaddr] || new Lease;
